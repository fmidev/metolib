describe("Cache", function() {
    ...
    it('should fill error gaps with NaN', function(){
        
        /* ========================================== */
        cache.addDataProvider('error', function(taskDef, callback) {
               var retval = {};
               var i = 0;
               async.forEach(taskDef.location, function(loc, locNotify) {
                   if (retval[loc] === undefined) {
         ...
         var taskDef = {
            ...
            waitsFor(function() {
             ...
             runs(function() {
                 _.each(this.result.data[taskDef.location][taskDef.parameter], function(value,index){
                     expect(value).toEqual(taskDef.start+index*taskDef.resolution);
                 });
                 expect(this.exception).toBeNull();
            ...
            waitsFor(function() {
             ...
             runs(function() {
                 var firstTimeStepForSecondFetch = taskDef.start+(taskDef.pointCount+Math.ceil(taskDef.pointCount*sideAfter))*taskDef.resolution;
                 _.each(this.result.data[taskDef2.location][taskDef2.parameter], function(value,index){
                     var time = taskDef2.start+index*taskDef2.resolution;                                          
                     if (time < firstTimeStepForSecondFetch){
                         //These are cached non-error values generated by the first request:
                         expect(value).toEqual(time);
                     }
                     else {
                         //These are error values generated by the second request
                         expect(value).toBeNaN();
                     }
                 });
                 expect(this.exception).toBeNull();
                 expect(this.err[0].start).toEqual(firstTimeStepForSecondFetch);
                 expect(this.err[0].end).toEqual(firstTimeStepForSecondFetch+(taskDef2.pointCount-1)*taskDef2.resolution);
                 expect(this.err[0].error).toEqual('test-error');
                 expect(this.result).not.toBeNull();
            });
